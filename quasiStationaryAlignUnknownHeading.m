...
Q = getQ_QuasiStationaryUnknown(state,imu,params,PHI);
...
%state.T = T_ZYX_Unknown2(state.dx(10),state.dx(11),state.dx(8),state.dx(9))'*state.T; % Errors may not yet be <2deg
%state.T = T_ZYX_Unknown(state.sinSi,state.cosSi,state.t(2),state.t(3)); % Errors may not yet be <2deg
...
%state.T = dT*state.T;
%state.T = recomputeT(state.sinSi,state.cosSi,state.dx(8),state.dx(9))'*state.T;
state.T = reOrthoNorm(state.T);
state.b_a = state.dx(12:14);
state.b_g = state.dx(15:17);
...
function dT = recomputeT(sinSi,cosSi,theta,phi)

dT = [ cosSi, sinSi, -theta;...
        -sinSi, cosSi, phi;...
        theta*cosSi + phi*sinSi, theta*sinSi - phi*cosSi, 1];
...
function T = T_ZYX_Unknown2(dsinSi,dcosSi,theta,phi)
% Define a Rotation Matrix for ZYX Euler sequence
cosTheta = cos(theta);
sinTheta = sin(theta);
cosPhi = cos(phi);
sinPhi = sin(phi);
Rz = [ dcosSi -dsinSi 0 ; dsinSi dcosSi 0 ; 0 0 1];  %yaw
Ry = [ cosTheta 0 sinTheta ; 0 1 0 ; -sinTheta 0 cosTheta]; %pitch
Rx = [ 1 0 0 ; 0 cosPhi -sinPhi ; 0 sinPhi cosPhi]; %roll
